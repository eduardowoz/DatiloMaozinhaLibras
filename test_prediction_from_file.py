...
if __name__ == "__main__":
    if not os.path.exists(MODEL_PATH) or not os.path.exists(LABELS_PATH):
        print("ERRO: Arquivos de modelo não encontrados. Rode 'trainingmodel.py' primeiro.")
        exit()
    
    print("--- Teste de Predição a Partir de Arquivo ---")
    
    target_letter = input("Digite a letra do dataset que você quer usar para o teste (ex: A, B, J): ").upper()
    ...
    try:
        target_sample_id = int(input("Digite o ID da amostra que você quer testar (copie da lista acima): "))
    except ValueError:
    ...
    sequence_to_predict = prepare_single_sequence(sample_data, FRAMES_PER_SAMPLE)
    ...
    print(f"Amostra Testada: '{target_sample_id}' (Letra Real: {target_letter})")
    print("-" * 30)
